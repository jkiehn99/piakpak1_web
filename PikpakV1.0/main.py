...
import lib.mail
from pikpak_core.result_store import join_result_fields
...
PROMOTING_BASE_URL = 'https://api-referral.mypikpak.com'
DEFAULT_CLIENT_ID = 'YUMx5nI8ZU8Ap8pm'
LOGIN_CLIENT_ID = 'YNxT9w7GMdWvEOKa'
LOGIN_CLIENT_SECRET = 'dbw2OtmVEeuUvIptb1Coyg'
LOGIN_PACKAGE_NAME = 'com.pikcloud.pikpak'
LOGIN_CLIENT_VERSION = '1.47.1'
LOGIN_SIGN_ALGORITHMS = [
...
def _log(msg, end='\n'):
    ...
    with _log_lock:
        ...
        if end == '':
            ...
            if _log_buffer:
                print(_console_safe_text(f'{_log_buffer}{msg}'), end='\n')
                _log_buffer = ''
            else:
                print(_console_safe_text(f'\u2502{ts}\u2502 {prefix}{msg}'), end='\n')
        sys.stdout.flush()
...
def calc_login_sign(device_id, timestamp, path):
    data = f'{LOGIN_CLIENT_ID}{LOGIN_CLIENT_VERSION}{LOGIN_PACKAGE_NAME}{device_id}{timestamp}{path}'
    for salt in LOGIN_SIGN_ALGORITHMS:
        data = hashlib.md5((data + salt).encode('utf-8')).hexdigest()
    return data


def signin_with_password(email, password, device_id=None, captcha_token=''):
    device_id = (device_id or '').strip() or generate_device_id()
    preferred_captcha_token = (captcha_token or '').strip()
    session = requests.Session()
    session.trust_env = False
    proxies = get_proxy_dict()

    def headers_for(path, captcha_token=''):
        timestamp = str(int(time.time() * 1000))
        sign = calc_login_sign(device_id, timestamp, path)
        headers = {
            'X-Client-Id': LOGIN_CLIENT_ID,
            'X-Device-Id': device_id,
            'X-Device-Sign': f'wdi10.{device_id}{sign}',
    ...
    init_status_code = 0
    init_data = {'skipped': True, 'reason': 'preferred_captcha_token'} if preferred_captcha_token else {}
    active_captcha_token = preferred_captcha_token

    if not active_captcha_token:
        init_resp = session.post(
            BASE_URL + '/v1/shield/captcha/init',
            headers=headers_for('/v1/shield/captcha/init'),
            json={
        ...
        init_status_code = init_resp.status_code
        try:
            init_data = init_resp.json()
        except Exception:
            init_data = {'raw': init_resp.text}
        active_captcha_token = init_data.get('captcha_token', '') if isinstance(init_data, dict) else ''

    signin_resp = session.post(
        BASE_URL + '/v1/auth/signin',
        headers=headers_for('/v1/auth/signin', active_captcha_token),
        json={
            'client_id': LOGIN_CLIENT_ID,
    ...
    try:
        ...
        signin_data = {'raw': signin_resp.text}

    captcha_retry_debug = {}
    if isinstance(signin_data, dict) and signin_data.get('error') == 'captcha_invalid' and active_captcha_token:
        captcha_retry_debug = {
            'stage': 'captcha_invalid',
            'device_id_prefix': device_id[:16],
            'init_token_prefix': active_captcha_token[:24],
            'used_preferred_captcha_token': bool(preferred_captcha_token),
            'attempts': []
        }
        max_captcha_retries = 3
        for captcha_attempt in range(1, max_captcha_retries + 1):
            attempt_debug = {'attempt': captcha_attempt}
            try:
                _debug(f'???????????????????????????????????captcha_token ({captcha_attempt}/{max_captcha_retries})')
                solved = protocol_solve_captcha()
                attempt_debug['solved'] = bool(solved)
                if solved:
                    verified_token = exchange_ticket_for_jwt(device_id, solved['ticket'], solved['randstr'], active_captcha_token, 'zh-CN')
                    attempt_debug['verified_token_prefix'] = verified_token[:24] if verified_token else ''
                    if verified_token:
                        active_captcha_token = verified_token
                        signin_resp = session.post(
                            BASE_URL + '/v1/auth/signin',
                            headers=headers_for('/v1/auth/signin', active_captcha_token),
                            json={
                        ...
                        try:
                            ...
                            signin_data = {'raw': signin_resp.text}
                        attempt_debug['retry_status_code'] = signin_resp.status_code
                        attempt_debug['retry_error'] = signin_data.get('error') if isinstance(signin_data, dict) else ''
                        attempt_debug['retry_error_code'] = signin_data.get('error_code') if isinstance(signin_data, dict) else ''
                        if signin_resp.status_code == 200 and isinstance(signin_data, dict) and signin_data.get('access_token'):
                            captcha_retry_debug['success_attempt'] = captcha_attempt
                            captcha_retry_debug['attempts'].append(attempt_debug)
                            break
                        if not (isinstance(signin_data, dict) and signin_data.get('error') == 'captcha_invalid'):
                            captcha_retry_debug['attempts'].append(attempt_debug)
                            break
                captcha_retry_debug['attempts'].append(attempt_debug)
                time.sleep(1.5 * captcha_attempt)
            except Exception as exc:
                attempt_debug['error'] = str(exc)
                captcha_retry_debug['attempts'].append(attempt_debug)
                captcha_retry_debug['error'] = str(exc)
                _debug(f'??????????????????????? {exc}')
                break
        if captcha_retry_debug.get('attempts'):
            attempts = captcha_retry_debug['attempts']
            last_attempt = attempts[-1]
            captcha_retry_debug['solved'] = last_attempt.get('solved', False)
            captcha_retry_debug['solved_any'] = any(bool(item.get('solved')) for item in attempts)
            captcha_retry_debug['review_after_solved'] = any(
                bool(item.get('solved')) and item.get('retry_error') == 'captcha_invalid'
                for item in attempts
            )
            captcha_retry_debug['verified_token_prefix'] = last_attempt.get('verified_token_prefix', '')
            captcha_retry_debug['retry_status_code'] = last_attempt.get('retry_status_code')

    return {
        'status_code': signin_resp.status_code,
        'data': signin_data,
        'device_id': device_id,
        'captcha_token': active_captcha_token,
        'init_status_code': init_status_code,
        'init_data': init_data,
        'captcha_retry_debug': captcha_retry_debug,
    }
...
def join_promoting_plan(access_token, captcha_token, device_id):
    return make_request('POST', PROMOTING_BASE_URL, '/promoting/v1/join', headers=_pikpak_headers(device_id, {
        'Authorization': f'Bearer {access_token}',
        'X-Captcha-Token': captcha_token,
        'Referer': 'https://mypikpak.com/',
        'Origin': 'https://mypikpak.com',
...
def refresh_access_token(refresh_token, device_id=None):
    device_id = device_id or generate_device_id()
    return make_request('POST', BASE_URL, '/v1/auth/token', headers=_pikpak_headers(device_id, {
        'Referer': 'https://mypikpak.com/',
        'Origin': 'https://mypikpak.com',
    }), body={
        'client_id': DEFAULT_CLIENT_ID,
...
def append_result(email, password, access_token='', refresh_token='', user_id='', invite_ok=None, pro_ok=None, captcha_token='', device_id=''):
    reg_time = time.strftime('%Y-%m-%d %H:%M:%S')
    invite_str = '成功' if invite_ok else ('失败' if invite_ok is False else '未绑定')
    pro_str = '成功' if pro_ok else ('失败' if pro_ok is False else '未升级')
    ...
    return {'email': email, 'password': password, 'access_token': access_token,
            'refresh_token': refresh_token, 'user_id': user_id, 'reg_time': reg_time,
            'invite': invite_str, 'pro': pro_str, 'captcha_token': captcha_token, 'device_id': device_id}
...
def run_batch_round(round_num):
    ...
    refresh_token = signup_resp['data'].get('refresh_token', '')
    user_id = signup_resp['data'].get('sub', '')
    if access_token and not refresh_token:
        _log('    ● rt缺失，登录补取...', end='')
        try:
            signin_resp = signin_with_password(mail_account['email'], password)
            signin_data = signin_resp.get('data') if isinstance(signin_resp, dict) else {}
            if signin_resp.get('status_code') == 200 and isinstance(signin_data, dict):
                refresh_token = signin_data.get('refresh_token', '') or refresh_token
                access_token = signin_data.get('access_token', '') or access_token
                user_id = signin_data.get('sub', '') or user_id
                _log(' ✓' if refresh_token else ' ✗')
            else:
                _log(' ✗')
                _debug(f'rt登录补取失败: {json.dumps(signin_data, ensure_ascii=False)}')
        except Exception as e:
            _log(' ✗')
            _debug(f'rt登录补取异常: {e}')
    _log(f'    \u25CF token: {access_token[:50]}...')
    if refresh_token:
        _log(f'    \u25CF rt: {refresh_token[:24]}...')
    _log(f'    \u25CF uid: {user_id}')
    ...
    if access_token:
        ...
        if _is_stopped(): return False
        try:
            pro_resp = join_promoting_plan(access_token, captcha_token, device_id)
            if pro_resp["status_code"] == 200:
                ...
                pro_ok = False
                _log(' \u2717')
            _debug(f'Pro升级结果: {"成功" if pro_ok else json.dumps(pro_resp["data"], ensure_ascii=False)}')
        except Exception as e:
            pro_ok = False
            _debug(f'Pro升级异常: {e}')
            _log(' \u2717')

    acct = append_result(mail_account['email'], password, access_token, refresh_token, user_id, invite_ok, pro_ok, captcha_token, device_id)
