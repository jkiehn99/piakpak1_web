...
def split_proxy_candidates(proxy_gateway):
...
def _has_session_param(proxy_url):
    return bool(re.search(r'(?i)(?:^|[-_])sessid-[A-Za-z0-9]+', proxy_url or ''))


def _rotate_session_param(proxy_url, worker_id=None):
    if not _has_session_param(proxy_url):
        return proxy_url

    seed = f'{int(time.time() * 1000)}{random.randint(1000, 9999)}'
    if worker_id is not None:
        seed = f'{seed}{int(worker_id):02d}'

    return re.sub(
        r'(?i)(sessid-)[A-Za-z0-9]+',
        r'\g<1>' + seed,
        proxy_url,
        count=1,
    )
...
class ProxyManager:
    ...
    def configure(self, gateway='', rotate_every=1):
        fixed = [normalize_proxy_url(u) for u in split_proxy_candidates(gateway)]
        with self._lock:
            self._gateway = fixed[0] if fixed else ''
            self._gateway_original = self._gateway
    ...
    def _init_pool_from_gateway(self, gateway):
        with self._pool_lock:
            ...
            if not gateway:
                return
            urls = split_proxy_candidates(gateway)
            if not urls:
                return
            fixed = []
            for u in urls:
                fixed.append(normalize_proxy_url(u))
            self._pool = fixed
    ...
    def acquire(self, worker_id):
        with self._pool_lock:
            ...
            gateway = _rotate_session_param(gateway, worker_id=worker_id)
            ...
            with self._pool_lock:
                if self._pool and worker_id is not None:
                    ...
                    if available:
                        ...
                        return proxy
            if _has_session_param(self._gateway_original):
                rotated = _rotate_session_param(self._gateway_original, worker_id=worker_id)
    ...
    def pin(self):
        existing = getattr(self._tls, 'pinned', None)
